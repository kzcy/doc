...
但是最后都败在了服务器、备案等等各种繁琐事情上。

最近发现Gitee也有GitHub类似的Pages功能，故各种攻略，尝试在Gitee上搭建Hexo博客。
...
npm info hexo                                            # 查看hexo安装信息
```

新建博客目录文件夹，在文件夹内运行Git bash，执行：
...
![1](https://gitee.com/kzcy/pic/raw/master/img/1.png)

### 安装主题

在[hexo官网](https://hexo.io/themes/)找一个自己心仪的主题，我们以流行的NexT主题为例。

```bash
git clone https://github.com/theme-next/hexo-theme-next themes/next
```

{% note info %}修改根目录的全局配置文件`_config.yml`,`theme`字段修改为主题名：`next`。{% endnote %}
...
1. 根目录的全局配置文件`_config.yml`；
2. `\hexo\themes\next`目录下的主题配置文件`_config.yml`。
...
即可[预览](http://localhost:4000)效果。

### 书写文章
```bash
hexo new test        # 在/source/_posts下生成名为test.md的文件
```

{% note info %}文章采用markdown语法，可以百度学习。{% endnote %}
...
- title                标题；
- date               日期，格式为：`2020-08-25 15:22:28`；
- tages              标签，每一行`- `后空一格填写便签名；
...
deploy:
  type: git
  repository: https://gitee.com/kzcy/kzcy.git
  branch: master
```
...
url: https://kzcy.gitee.io  # 填写博客地址
root: /
...
social:
  ...
  E-Mail: mailto:kzcy@foxmail.com || fa fa-envelope
  Weibo: https://weibo.com/312218335 || fab fa-weibo
